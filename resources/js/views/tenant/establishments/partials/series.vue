...
export default {
    ...
    methods: {
        ...
        validar_number(index) {
            ...
            let contingency = this.records[index].contingency;
            let inicial = number.substr(0, 1);
            if (number.length != 4) {
                this.$message.error("El Nº Serie debe tener 4 Digitos");
                this.view_check = false;
            } else {
                if (document_type_id == "01" && !contingency) {
                    //F
                    if (inicial != "F") {
                        this.view_check = false;
                        this.$message.error(
                            "Letra inicial para Factura debe ser debe F"
                        );
                ...
                if (document_type_id == "03" && !contingency) {
                    //B
                    if (inicial != "B") {
                        this.view_check = false;
                        this.$message.error(
                            "Letra inicial para Boleta de Venta debe ser debe B"
                        );
                        //this.records[index].number = "";
                    } else {
                        this.view_check = true;
                ...
                if (document_type_id == "09") {
                    if (inicial != "T") {
                        this.view_check = false;
                        this.$message.error(
                            "Letra inicial debe ser T0 para Guia de Remitente"
                        );
                        //   this.records[index].number = "";
                    } else {
                        this.view_check = true;
                ...
                if (document_type_id == "31") {
                    if (inicial != "V") {
                        this.view_check = false;
                        this.$message.error(
                            "Letra inicial debe ser V0 para Guia de Transportista"
                        );
                        //this.records[index].number = "";
                    } else {
                        this.view_check = true;
        ...
        clickSubmit(index) {
            let form = {
                ...
                internal: this.records[index].internal,
        ...
        initDocumentTypes() {
            this.document_types =
                this.all_document_types.length > 0
                    ? this.all_document_types
                    : [];
        },
        close() {
